...
ExifViewerUI.updateView = function (id, exif, code)
{
	...
	//lite.css("top", (imageOffset.top + imageHeight - lite.height()) + unit);
	//lite.css("left", (imageOffset.left + imageWidth - lite.width()) + unit);
	ev.log("已为 %s 更新视图", exifId);
}
...
ExifViewerUI._getTemplateLite = function (object)
{
	...
	//template.css("top", (offset.top + height - template.height()) + unit);
	//template.css("left", (offset.left + width - template.width()) + unit);
...
{
	var gpsMap = $("#exifviewerui-gps-map");
	gpsMap.stop().fadeTo(200, 0, function () { gpsMap.css("display", "none"); });
}
...
ExifViewerUI.viewLite_mouseOutHandler = function (e)
{
	...
	var lite = $(target);
	lite.find("a.exifviewerui-gps-link").css("opacity", 1)
	lite.stop().fadeTo(200, 0, function () { lite.css("display", "none"); } );
}
...
{
	var object = $( 
		 '<div id="notifyVersionView" style="z-index: 99999; margin: 0; background: #F2B600; border: 1px solid #CC9900; color: #4F3C00; padding: 8px; font: 12px/12px Arial, sans-serif; position: absolute; left:0; right: 0; "><div style="float: left; ">'
		 + chrome.i18n.getMessage("WorkingNotify", ExifViewerConfig.manifest.version)
		 +  '</div>'
		 + '<div style="float: right;"><a href="'+ chrome.extension.getURL("options.html") + '">' + chrome.i18n.getMessage("GotoOptionsPage") + '</a></div>'
		 + '<br clear="both"/></div>');
