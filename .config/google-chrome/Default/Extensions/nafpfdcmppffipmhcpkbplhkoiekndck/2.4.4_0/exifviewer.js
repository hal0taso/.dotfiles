...
// ExifViewer.logLevel = ExifViewer.LOG_LOG | ExifViewer.LOG_ERROR;
...
ExifViewer._img_eachHandler1 = function (index, element)
{
	...
	ev.log("已为第 %s 个 img 补充 ID：%s，src=%s", index, element.id, element.src);
...
ExifViewer._img_exifviewerimageonloadhandler = function (e)
{
	...
	ev.log("%s 已下载完毕", element.id);
	var object = $(element);
	if (object.width() >= ExifViewerConfig.getEntry("minImageWidth")
	 && object.height() >= ExifViewerConfig.getEntry("minImageHeight"))
	{
...
ExifViewer.endGetExif = function (id, exif, code)
{
	...
	ev.log("已接收到 %s 的 EXIF %s", id, exif);
}
...
ev.log("已注入 exifviewer.js (%s)", location.href);